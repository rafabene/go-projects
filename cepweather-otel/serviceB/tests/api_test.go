...
func TestValidInputNoCeP(t *testing.T) {
	jsonStr := `{"nome":"Rafael"}` // Invalid JSON - missing cep field
	r := httptest.NewRequest("POST", "/api/v1/weather", strings.NewReader(jsonStr))
	w := httptest.NewRecorder()
...
func TestCepIncomplete(t *testing.T) {
	jsonStr := `{"cep":"75110"}` // Invalid JSON - not enough digits
	r := httptest.NewRequest("POST", "/api/v1/weather", strings.NewReader(jsonStr))
	w := httptest.NewRecorder()
...
func TestCepWrongSize(t *testing.T) {
	jsonStr := `{"cep":"751104301"}` // Invalid JSON - extra digits
	r := httptest.NewRequest("POST", "/api/v1/weather", strings.NewReader(jsonStr))
	w := httptest.NewRecorder()
...
func TestValidInput(t *testing.T) {
	jsonStr := `{"cep":"75110430"}` // Valid JSON with correct CEP
	r := httptest.NewRequest("POST", "/api/v1/weather", strings.NewReader(jsonStr))
	w := httptest.NewRecorder()
...
func TestInvalidCep(t *testing.T) {
	jsonStr := `{"cep":"75123456"}` // Cep invalid - not found
	r := httptest.NewRequest("POST", "/api/v1/weather", strings.NewReader(jsonStr))
	w := httptest.NewRecorder()
